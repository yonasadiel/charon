...
func CreateRouter() (router *mux.Router) {
	...
	router.HandleFunc("/exam/{eventID}/question/", helios.WithMiddleware(exam.QuestionCreateView, loggedInMiddlewares)).Methods(http.MethodPost)
	...
	router.HandleFunc("/exam/{eventID}/question/{questionID}/", helios.WithMiddleware(exam.QuestionDeleteView, loggedInMiddlewares)).Methods(http.MethodDelete)
